...
def install_ruby
  ...
  when /darwin|linux-gnu/
    return if system('command -v ruby >/dev/null 2>&1')

    puts "Installing Ruby using RVM..."
    execute_command('curl -sSL https://get.rvm.io | bash -s stable --ruby')
    execute_command('source /usr/local/rvm/scripts/rvm')
  when /mswin|mingw|cygwin/
    return if system('command -v ruby >/dev/null 2>&1')

    puts "Installing Ruby using Chocolatey..."
    execute_command('choco install ruby')
  else
    puts "Unsupported OS: #{RbConfig::CONFIG['host_os']}. Cannot install Ruby."
    exit(1)
  end
end
